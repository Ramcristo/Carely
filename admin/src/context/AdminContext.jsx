...
const AdminContextProvider = (props) => {
  ...
  const cancelAppointment = async (appointmentId) => {
       try {
        const {data} = await axios.post( "http://localhost:8000/api/admin/cancel-appointment",{appointmentId},{headers:{adminToken}})
        if(data.success){
          toast.success(data.message)
          getAllAppointments()
        }else{
  ...
  const getDashData = async () => {
    try {
      const {data} = await axios.get("http://localhost:8000/api/admin/dashboard",{headers:{adminToken}})
      if(data.success){
