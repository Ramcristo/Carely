...
const MyProfile = () => {
  ...
  const updateUserProfileData = async () => {
       try {
        ...
        const {data} = await axios.post('http://localhost:8000/api/user/update-profile',formData,{headers:{token}})
        if(data.success){
      ...
      {/* This conditional rendering for Phonenumber (User can change the phonenumber)*/}     
      {
       ...
       : <p className='text-primary'>{userData.phone}</p>
          ...
          {/* This conditional rendering for Address(User can change the address)*/}
          {
            isEdit
            ?<p>
              <input type="text" value={userData.address.line1} onChange={(e) => setUserData(previousData=> ({...previousData,address:{...previousData, line1: e.target.value}}))} />
              <input type="text" value={userData.address.line2} onChange={(e) => setUserData(previousData=> ({...previousData,address:{...previousData, line2: e.target.value}}))} />
            </p>
